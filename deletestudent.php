...
else
{
  ...
  if(!password_verify($pass, $out['password']))
  {
    ...
    if($out['teacher']==1)
    {
      ...
      $newout=array_diff($newout,[$_COOKIE['username']]);
      $result=mysqli_query($conn,"UPDATE students SET teachers=\"" .implode($newout,","). "\" WHERE username=\"" .$_GET['username']. "\"") or die(mysqli_error($conn));
    }
  }
}
 ?>
